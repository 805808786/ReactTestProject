...
export default function SceneEnterpriseDynamicDetail() {
  ...
  async function init(detailId) {
    const fetchDetail = async () => {
      try {
        const response = await getNewsInsightDetail({ id: detailId });
    ...
    const fetchAllRelatedNews = async () => {
      try {
          ...
          <div className="sedd-card-content">
            ...
            <div className="sedd-info-section">
              ...
              <div className="sedd-badge-wrapper">
                <div
                  className={`sedd-badge ${getBadgeClass(data.type, data.category)}`}
                >
                  {data.type}
                </div>
              </div>
              ...
              <div className="sedd-title-row">
                <div className="sedd-title-inner">
                  <div
                    className={`sedd-green-dot ${getGreenDotClass(data.type, data.category)}`}
                  />
            ...
            {data.paragraphs && data.paragraphs.length > 0 ? (
              <div className="sedd-paragraphs">
                {data.paragraphs.map((para, index) => (
                  <p key={index} className="sedd-paragraph">
                    {para}
                  </p>
                ))}
              </div>
          ...
          <div className="sedd-card">
            <div className="sedd-card-content">
              ...
              <div className="sedd-section-header">
                ...
                <span className="sedd-section-count">
                  ...
                  <div key={department.id} className="sedd-company-item">
                    ...
                    <div className="sedd-company-main">
                      ...
                      <div className="sedd-company-left">
                        <div className="sedd-department-name">
                          {department.name}
                        </div>
                      </div>
                      {/* 右侧：状态标签 */}
                      <div className="sedd-person">
                ...
                {data.relatedCompanies.map((company) => (
                  <div key={company.id} className="sedd-company-item">
                    ...
                    <div className="sedd-company-main">
                      ...
                      {company.id && (
                        <div
                          className="sedd-company-detail-row"
                          onClick={() =>
                            navigate(`/company-detail/${company.id}`)
                          }
                        >
                          <span className="sedd-company-detail-link">
                            查看详情 →
                          </span>
                        </div>
                      )}
